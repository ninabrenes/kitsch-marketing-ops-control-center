...
const metrics: readonly Metric[] = [
  {
    ...
    fields: ['Customer ID + first-order date', 'Order ID + cancellation status', 'Media spend by channel/campaign', 'Agency, creator, affiliate + tool costs', 'Currency + daily FX rate'],
    decision: 'Set an allowable CAC from contribution LTV and payback; separate media CAC from fully loaded CAC before scaling.',
    confidence: 'MEDIUM',
    limitation: '40K+ ecommerce ad accounts and $21B spend. The published CPA may include repeat purchasers and is not a fully loaded CAC.',
    source: 'https://www.triplewhale.com/blog/whats-a-good-cost-per-acquisition',
  ...
  },
  {
    ...
    formula: 'ROAS = attributed net revenue ÷ ad spend · MER = total net revenue ÷ total marketing spend',
    fields: ['Spend + campaign/ad/creative IDs', 'Attributed order ID + model/window', 'Gross sales, discounts + reversals', 'Customer type: new or returning', 'Currency + daily FX rate'],
    decision: 'Use platform ROAS to diagnose; use MER and contribution after marketing to hold, fix, test, fund or stop spend.',
    confidence: 'MEDIUM',
    limitation: 'Global cross-category medians, not beauty targets. Attribution models can move revenue credit between channels. Break-even ROAS must come from Kitsch margin.',
    source: 'https://www.triplewhale.com/blog/whats-a-good-cost-per-acquisition',
  ...
  },
  {
    ...
    question: 'How much product revenue remained after discounts and reversals?',
    reference: 'No universal $ benchmark',
    referenceLabel: 'Compare to plan, prior year and like-for-like channel/SKU cohorts',
    formula: 'Gross sales − discounts − sales reversals',
    fields: ['Order + line-item IDs', 'Order, refund + reversal timestamps', 'Channel, market, product, variant + SKU', 'Gross line sales + discount allocation', 'Refund/reversal value + currency/FX'],
    decision: 'Forecast demand, pace budgets, identify price/mix changes and reconcile commerce reporting to finance.',
    confidence: 'HIGH',
    limitation: 'Shopify net sales excludes tax and shipping. Recognized revenue remains finance-owned; marketplace and retail feeds need the same contract.',
  ...
  },
  {
    ...
    question: 'How efficiently does qualified traffic become a completed purchase?',
    reference: '2.70% – 4.94%',
    referenceLabel: 'Global skincare / beauty references · 2025–2026 publications',
    formula: 'Sessions that completed checkout ÷ eligible store sessions',
    fields: ['Session ID + timestamp', 'Completed-checkout / purchase event', 'Landing page + device', 'Source, medium, campaign + creative', 'Market + consent/tracking state'],
    decision: 'Locate a traffic-quality, product-page, cart or checkout constraint before buying more reach.',
    confidence: 'MEDIUM',
    limitation: 'Two different category datasets and methodologies form the band. Orders can exceed converted sessions; Shopify and GA4 attribution can differ.',
  ...
  },
  {
    ...
    question: 'Does each order create enough cash contribution to support growth?',
    reference: '51.04% gross · 11.68% net',
    referenceLabel: 'US Household Products public-company proxy · January 2026',
    formula: 'Gross margin = (net sales − COGS) ÷ net sales · CM1 = net sales − COGS − variable fulfillment/fees',
    fields: ['Realized net sales by line', 'SKU cost + effective date', 'Pick/pack, postage + duties', 'Payment + marketplace fees', 'Returns handling + variable service cost'],
    decision: 'Calculate break-even ROAS and the maximum CAC; distinguish high revenue from profitable revenue.',
    confidence: 'MEDIUM',
    limitation: 'NYU sector proxy covers 110 US public companies, not comparable DTC stores. Kitsch channel mix, wholesale economics and cost accounting will differ.',
  ...
  },
  {
    ...
    question: 'How much contribution does an acquisition cohort realize over time?',
    reference: 'No defensible universal $ band',
    referenceLabel: 'Directional guardrail: ~3:1 LTV:CAC only after definitions match',
    formula: 'Cohort cumulative contribution at 90/180/365 days ÷ customers acquired in that cohort',
    fields: ['Durable customer ID + first-order cohort', 'All orders, dates + line items', 'Discounts, refunds + reversals', 'COGS, fulfillment + transaction fees', 'Acquisition channel + cohort CAC'],
    decision: 'Set CAC ceilings, payback expectations and retention investment by first product, channel and customer cohort.',
    confidence: 'MEDIUM',
    limitation: 'External dollar LTV is meaningless without price, margin, repeat window and censoring rules. Label revenue LTV and contribution LTV separately.',
  ...
  },
  {
    ...
    question: 'Did first-time buyers form a habit—or was the first order the finish line?',
    reference: '17%',
    referenceLabel: 'Health & Beauty second-purchase reference · 180 days',
    formula: 'Mature first-order customers with a second valid order within 180 days ÷ all eligible first-order customers',
    fields: ['Durable customer ID', 'First + second valid-order dates', 'Order status + cancellation/refund', 'First product/SKU + acquisition channel', 'Cohort maturity / observation cutoff'],
    decision: 'Find first-product and channel cohorts that deserve replenishment, education, bundles or suppression.',
    confidence: 'MEDIUM–LOW',
    limitation: 'The provider does not disclose the public sample size, geography or measurement period. Never include cohorts that have not had 180 days to mature.',
  ...
  },
  {
    ...
    question: 'Which lifecycle messages produce action without exhausting consent?',
    reference: '$0.10 → $0.97',
    referenceLabel: 'Email campaign RPR · average to top 10% · 2025',
    formula: 'Attributed realized net revenue ÷ delivered recipients',
    fields: ['Message + flow/campaign ID', 'Profile ID + consent state', 'Delivered, clicked + unsubscribed timestamps', 'Attributed order ID + model/window', 'Realized net revenue + bot filtering'],
    decision: 'Prioritize segmented flows over batch sends; diagnose relevance with clicks/orders and protect the audience with unsubscribe guardrails.',
    confidence: 'HIGH',
    limitation: 'Klaviyo analyzed billions of messages across ecommerce, not only beauty. Compare campaign with campaign and flow type with the same flow type; opens are privacy-inflated.',
    source: 'https://klaviyocms.wpengine.com/wp-content/uploads/2025/02/2025-Benchmark-Report_AMER.pdf',
  ...
  },
  {
    ...
    question: 'Is revenue contractually recurring—or simply reported every month?',
    reference: 'N/A for non-subscription orders',
    referenceLabel: 'Use monthly net revenue, repeat share and cohort LTV instead',
    formula: 'Subscription MRR only = sum of monthly-equivalent value of active recurring contracts',
    fields: ['Subscription/contract ID', 'Active, paused + cancelled status', 'Start/end/cancellation dates', 'Billing cadence + recurring amount', 'One-time versus recurring order flag'],
    decision: 'Prevent a SaaS metric from overstating retail predictability. Use MRR only for a real subscription program.',
    confidence: 'HIGH',
    limitation: 'A customer buying twice does not create MRR. Ordinary Shopify orders, retailer POs and forecasted sales are not contractual recurring revenue.',
...
const operationalTargets = [
  ['Launch on-time rate', '≥95%', 'Launches live by committed date with every critical gate passed ÷ launches due'],
  ['Milestone reliability', '≥90%', 'Due tasks completed on time ÷ tasks due'],
  ['Decision closure', '≤3 days', 'Median business days from logged question to recorded decision, DRI and deadline'],
  ['Action follow-through', '≥90%', 'Actions completed by agreed due date ÷ actions due'],
  ['Monthly budget variance', '±5%', '(Actual or accrued spend − approved plan) ÷ approved plan'],
  ['Forecast accuracy', '≥90%', '1 − |actual − forecast| ÷ actual'],
  ['Dashboard freshness', '<24 hr', 'Current time − latest successful source refresh'],
  ['Revenue reconciliation', '<1%', '|commerce net sales − finance-controlled value| ÷ finance-controlled value after close'],
] as const;

function Confidence({ value }: { value: Metric['confidence'] }) {
  return <span className={`mbl-confidence ${value === 'HIGH' ? 'is-high' : value === 'MEDIUM' ? 'is-medium' : 'is-low'}`}>{value} CONFIDENCE</span>;
}

export function MetricBenchmarkLab() {
  return <section className="mbl-shell">
    <style>{styles}</style>
    <header className="mbl-hero">
      <div className="mbl-hero-copy">
        <span className="mbl-kicker"><BarChart3 /> METRIC DECISION LAB</span>
        <h2>Numbers that tell the team what to do next—not just what happened.</h2>
        <p>Every metric has one definition, one source contract and one decision. Connect Shopify, Klaviyo, paid media and finance to unlock Kitsch actuals.</p>
      </div>
      <div className="mbl-hero-orbit" aria-label="Metric workflow">
        <span><Database /> Connect</span><ArrowUpRight />
        <span><Calculator /> Calculate</span><ArrowUpRight />
    ...
    <aside className="mbl-disclosure">
      <ShieldCheck />
      <div><strong>Benchmark ≠ Kitsch performance</strong><p>External bands are directional comparison points from the named source—not estimates, forecasts or claims about Kitsch. Every internal actual remains intentionally blank until governed first-party data is connected and reconciled.</p></div>
      <span>PUBLIC REFERENCE · INTERNAL ACTUAL REQUIRED</span>
    </aside>

    <div className="mbl-toolbar">
      <div><span>9</span><small>metric contracts</small></div>
      <div><span>0</span><small>invented Kitsch actuals</small></div>
      <div><span>1</span><small>decision owner per KPI</small></div>
      <p><Gauge /> Read each card left to right: actual → reference → formula → action.</p>
    </div>

    <div className="mbl-grid">
      {metrics.map((metric) => {
        const Icon = metric.icon;
        return <article className="mbl-card" key={metric.id}>
          <header>
            <div className="mbl-icon"><Icon /></div>
            <div><span>{metric.id}</span><h3>{metric.name}</h3></div>
            <Confidence value={metric.confidence} />
          </header>
          <p className="mbl-question">{metric.question}</p>
          <div className="mbl-values">
            <div className="mbl-actual"><span>INTERNAL ACTUAL</span><strong>—</strong><small><LockKeyhole /> Awaiting governed data</small></div>
            <div className="mbl-reference"><span>EXTERNAL REFERENCE</span><strong>{metric.reference}</strong><small>{metric.referenceLabel}</small></div>
          </div>
          <div className="mbl-formula"><Calculator /><div><span>FORMULA</span><p>{metric.formula}</p></div></div>
          <div className="mbl-fields"><span><Database /> EXACT INTERNAL FIELDS</span><div>{metric.fields.map((field) => <small key={field}>{field}</small>)}</div></div>
          <div className="mbl-decision"><Target /><div><span>DECISION IT CHANGES</span><p>{metric.decision}</p></div></div>
          <div className="mbl-limit"><CircleAlert /><p>{metric.limitation}</p></div>
          <a className="mbl-source" href={metric.source} target="_blank" rel="noopener noreferrer"><span>{metric.sourceLabel}</span><ExternalLink /></a>
        </article>;
      })}
    </div>

    <section className="mbl-ops">
      <header><div><span>ILLUSTRATIVE INTERNAL TARGETS</span><h3>The operating system should measure its own reliability.</h3><p>These are proposed starting thresholds—not public beauty-industry benchmarks and not claims about current Kitsch performance. Baseline for one quarter, then ratify targets with leadership.</p></div><Clock3 /></header>
      <div className="mbl-ops-grid">{operationalTargets.map(([name, target, formula], index) => <article key={name}><span>{String(index + 1).padStart(2, '0')}</span><div><h4>{name}</h4><p>{formula}</p></div><strong>{target}</strong></article>)}</div>
    </section>

    <footer className="mbl-footer"><ShieldCheck /><p><strong>Decision rule:</strong> never scale from an external benchmark alone. First verify identity coverage, attribution window, refund maturity, inventory and realized contribution.</p></footer>
  </section>;
}

const styles = `
.mbl-shell{--ink:#231F20;--muted:#53565A;--line:#D9D9D6;--paper:#F0E6D8;--blush:#E9D5CD;--coral:#CA9A8E;--wine:#713F2A;--sage:#CBDEDA;color:var(--ink);display:grid;gap:18px;margin-top:22px;font-family:inherit}
.mbl-shell *{box-sizing:border-box}.mbl-hero{background:linear-gradient(130deg,#261e1c 0%,#51332f 58%,#9e5044 100%);border-radius:26px;color:#fff;display:grid;grid-template-columns:1.2fr .8fr;gap:28px;min-height:270px;overflow:hidden;padding:38px;position:relative}.mbl-hero:after{background:radial-gradient(circle,rgba(255,255,255,.14) 0 2px,transparent 2.5px);background-size:19px 19px;content:"";inset:0 0 0 58%;opacity:.5;position:absolute}.mbl-hero-copy{position:relative;z-index:1}.mbl-kicker{align-items:center;color:#f7c9be;display:flex;font-size:11px;font-weight:800;gap:8px;letter-spacing:.15em}.mbl-kicker svg{height:16px;width:16px}.mbl-hero h2{font-family:var(--font-display,inherit);font-size:clamp(30px,4vw,52px);letter-spacing:-.035em;line-height:1.02;margin:18px 0 16px;max-width:760px}.mbl-hero p{color:#eadedb;font-size:15px;line-height:1.65;margin:0;max-width:680px}.mbl-hero-orbit{align-content:center;display:grid;gap:11px;justify-content:center;position:relative;z-index:1}.mbl-hero-orbit>span{align-items:center;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.2);border-radius:999px;display:flex;font-size:12px;font-weight:750;gap:9px;min-width:180px;padding:12px 18px}.mbl-hero-orbit svg{height:16px;width:16px}.mbl-hero-orbit>svg{color:#f4b3a5;margin-left:82px;transform:rotate(90deg)}
